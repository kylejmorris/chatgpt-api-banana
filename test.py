...
def chat(msg, conversation_id=None): 
    res = requests.post('http://localhost:8000', json={'prompt': msg, 'session_id': session_id, conversation_id: conversation_id})
    res = res.json()
    print(res)
    return res['message'], res['conversation_id']

response, conversation_id = chat("What is the most important topic in chemistyr?")
print(response)
print(conversation_id)
response = chat("can you list one more example?", conversation_id)
print(response)
